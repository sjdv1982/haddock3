...
1. **Did the content of anything CNS reads change?** Then it must be
   ...
   CNS script, the CNS executable itself.
...
1. **Phase 0** (`test_phase0_hardlink.py`) checks the *measuring instrument*,
   not the feature. If it fails, none of the rest means anything, so Phase 2
   refuses to run. This part is written for developers and is not worth your
   time as a reviewer.
...
| `test_phase0_hardlink.py` | Phase 0: checks the instrument itself |
...
The design documents behind all of this are
[`caching-use-cases.md`](../../caching-use-cases.md) (what must be reused and
why) and [`caching-test-suite-plan.md`](../../caching-test-suite-plan.md) (how
that is observed).
