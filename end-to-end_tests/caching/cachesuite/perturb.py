...
def _edit_models(path: Path, spec: dict, context: Context) -> None:
    ...
    if len(blocks) < 2:
        raise PerturbationError(f"{path} is not a multi-model ensemble")
...
def _op_cns_wrapper(spec: dict, context: Context) -> None:
    ...
    this platform.  A wrapper is a genuinely different executable that
    computes the same thing -- which is exactly the discriminating case: the
