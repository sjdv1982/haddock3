...
CNS_PSF_CANONICAL_FILENAME = b'; FILENAME="canonical-output.psf"'
...
def _normalize_psf_line(line: bytes) -> bytes:
    """Return a PSF line with filename-bearing title text canonicalized."""
    if line.strip().startswith(b"; FILENAME="):
        return CNS_PSF_CANONICAL_FILENAME + _line_ending(line)
...
def is_normalized_cns_artifact(
    ...
    stripped = _strip_gzip_suffix(Path(name).name)
...
def _strip_gzip_suffix(name: str) -> str:
    """Strip a trailing gzip suffix from an artifact name."""
    if name.endswith(".gz"):
        return name[:-3]
    return name
