...
# The configured CNS path is an execution requirement, but deliberately not a
# cache input.  Users asked for optimistic cache reuse when changing wrappers
# or executables because equivalence cannot be established reliably.
_CNS_EXECUTABLE_POLICY_CHECKSUM = str(
    calculate_checksum(b"haddock-cache-cns-executable-is-not-an-input")
)
...
def build_canonical_mapping(
    ...
    *,
    ...
    cns_exec_checksum = _CNS_EXECUTABLE_POLICY_CHECKSUM
...
def _rewrite_canonical_script(
    ...
    for candidate, name in sorted(
        ...
        if candidate:
            result = _replace_script_token(result, candidate, name)
    # Refinement replicas use ``count`` only as an output locator.  In
    # topology scripts it can instead distinguish real ensemble members
    # (for example ``..._1`` from ``..._11``), so never erase it globally.
    if module_dir.parent.name in {"topoaa", "topocg", "flexref", "emref", "mdref"}:
        result = _canonicalize_locator_count_assignment(result)
    if module_dir.parent.name in {"topoaa", "topocg"}:
        result = re.sub(
            r"(?im)(eval(?:uate)?\s*\(\s*\$seed\s*=\s*)[^)]+(\))",
            r"\1canonical-topology-seed\2",
            result,
        )
...
def _canonicalize_default_molecule_flags(script: str) -> str:
    """Drop generated false molecule flags which are CNS defaults.
    ...
    assignments are semantically inert and must not make an otherwise
    identical refinement job miss the cache.
    """
    return re.sub(
        r"(?im)^\s*eval\s*\(\s*\$mol_(?:fix_origin|shape)_\d+\s*=\s*false\s*\)\s*\n?",
        "",
        script,
    )
